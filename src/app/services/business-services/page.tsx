...
import { ArrowLeft, Phone, CheckCircle, Building } from "lucide-react";
...
async function getBusinessServices() {
  const [services, categories] = await Promise.all([
    client.fetch<Service[]>(servicesByCategoryQuery, { category: 'business-services' }),
    client.fetch<ServiceCategory[]>(serviceCategoriesQuery),
  ]);

  const businessCategory = categories.find(cat => cat.slug.current === 'business-services');
  
  return { services, businessCategory };
}

export default async function BusinessServicesPage() {
  const { services, businessCategory } = await getBusinessServices();
  return (
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          <Badge variant="secondary" className="mb-4 bg-va-secondary text-va-neutral-50 font-montserrat">
            {businessCategory?.title || 'Business Services'}
          </Badge>
          <h1 className="text-4xl font-montserrat font-bold mb-4 text-va-text-primary">
            {businessCategory?.title || 'Enterprise IT Solutions & Support'}
          </h1>
          <p className="text-lg text-va-text-secondary font-roboto">
            {businessCategory?.description || 'Comprehensive IT services designed to keep your business running smoothly and securely'}
          </p>
        </div>
...
{services.length === 0 ? (
          ...
          <div className="text-center py-16">
            <Building className="h-16 w-16 mx-auto text-va-text-muted mb-6" />
            ...
            <p className="text-va-text-secondary mb-8 max-w-2xl mx-auto font-roboto">
              We&apos;re updating our business service offerings. Please contact us directly for information about our enterprise IT solutions.
            </p>
                      ...
                      <div className="text-sm text-va-text-muted">
                        ...
                        {service.pricing?.hourlyRate && (
                          <div className="text-xs">
                            ${service.pricing.hourlyRate}/hour
                          </div>
                        )}
                      </div>
